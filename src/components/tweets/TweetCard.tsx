...
const TweetCard: React.FC<TweetCardProps> = ({ tweet, compact = false, onClick }) => {
  const handleClick = () => {
    if (onClick) onClick(tweet);
  };
  ...
  return (
    <Card 
      className={`border hover:shadow-md transition-shadow duration-300 ${onClick ? 'cursor-pointer' : ''}`}
      onClick={handleClick}
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1">
              <p className="font-semibold truncate">{tweet.user.name}</p>
              {tweet.user.verified && (
            ...
            <p className="text-gray-500 text-sm truncate">{tweet.user.username}</p>
          </div>
          
          <div className="text-gray-500 text-xs whitespace-nowrap">{formattedDate}</div>
        </div>
      </CardHeader>
      ...
      <CardFooter className="px-4 pt-0 pb-3 border-t">
